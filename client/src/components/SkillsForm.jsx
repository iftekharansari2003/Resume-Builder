...
const SkillsForm = ({data,onChange}) => {
    ...
    const removeSkill=(idx)=>{
        onChange(data.filter((_, index)=>index!==idx))
    }
  ...
  return (
    <div className='space-y-4'>
        ...
        <div className='flex gap-2'>
            <input type="text" placeholder='Enter a skill (e.g., Java, Blockchain)' className='flex-1 px-3 py-2 text-sm' onChange={(e)=>setNewSkill(e.target.value)} value={newSkill} onKeyDown={handleKeyPress}/>
            ...
            <div className='flex flex-wrap gap-2'>
                {data.map((skill,idx)=>(
                    <span key={idx} className='flex items-center gap-1 px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm'>
                        {skill}
                        <button onClick={()=>removeSkill(idx)} className='ml-1 hover:bg-blue-200 rounded-full p-0.5 transition-colors'>
