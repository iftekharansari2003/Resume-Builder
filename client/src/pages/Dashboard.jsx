...
import {FilePenIcon, FilePenLineIcon, LoaderCircleIcon, PencilIcon, PlusIcon, TrashIcon, UploadCloudIcon, UploadIcon, XIcon,FileText } from 'lucide-react'
...
const editTitle = async(e)=>{
    try {
      ...
      setAllResume(allResumes.map(resume=>resume._id===editResumeId ? {...resume,title}:resume))
      setEditResumeId("");
      setTitle("");
  ...
  return (
    <div>
      <div className='max-w-7xl mx-auto px-4 py-8'>
        ...
        <p className='text-2xl font-medium mb-6 bg-gradient-to-r from-slate-600 to-slate-700 bg-clip-text text-transparent sm:hidden'>Welcome, Iftekhar Ansari</p>
    ...
    <h2 className="text-lg md:text-xl font-semibold text-blue-800 tracking-wide">
      Existing Resume
        ...
        {allResumes.map((resume,idx)=>{
          ...
          return(
            <button key={idx} onClick={()=>navigate(`/app/builder/${resume._id}`)} className='relative w-full sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 border group hover:shadow-lg transition-all duration-300 cursor-pointer' style={{background:`linear-gradient(135deg, ${baseColor}10, ${baseColor}40)`, 
        ...
        className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
          <div onClick={e=>e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
            ...
            <div>
              ...
              <input type="file" accept='.pdf' id="resume-input"  hidden onChange={(e)=>setResume(e.target.files[0])}/>
            </div>
